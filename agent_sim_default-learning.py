...
class LearningAgent(Agent):
    ...
    def reset(self, destination=None, testing=False):
        ...
        #self.epsilon = 0.99**self.n_trials
    ...
    def choose_action(self, state):
        ...
        if not self.learning:
            ...
            indices = [i for i, x in enumerate(self.Q[state].values()) if x == self.get_maxQ(state)]
            '''
            print 'state'
            print self.Q[state]
            print 'keys'
            ...
            print 'values'
            print self.Q[state].values()
            print 'maxQ'
            print self.get_maxQ(state)
            #print self.Q[state].values().index(self.get_maxQ(state))
            print indices
            '''
            random_index = random.choice(indices)
            action = self.Q[state].keys()[random_index]
        return action
...
def run():
    ...
    #sim = Simulator(env, log_metrics=log_metrics, update_delay=update_delay, display=display, optimized=optimized)
    sim = Simulator(env, log_metrics=log_metrics, update_delay=update_delay, display=display)
    #sim = Simulator(env)
